...
class Agent:
  ...
  def __init__(self,
    ...
    self.policy_freq = 1               # delayed policy updates (1 = every iteration)
  ...
  def _update_critic(self, batch):
    ...
    with torch.no_grad():
      ...
      target_q = torch.min(target_q1, target_q2) - torch.exp(self.log_alpha) * next_logp
      target = rew.unsqueeze(-1) + (1.0 - done.unsqueeze(-1)) * self.gamma * target_q
  ...
  def train(self, make_env_fn, total_steps: int = 3_000_000,
    ...
    while self.total_env_steps < total_steps:
      ...
      if (self.total_env_steps > self.update_after and
        ...
        batch = {k[:3] if k.startswith('obs') else k[:4]: v for k, v in batch.items()}
